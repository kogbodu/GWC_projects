...
class Building():
	...
	def draw(self):
		...
		#height2 = random.randrange(300,400)
		#height3 = random.randrange(500,550)
		#width = SCREEN_WIDTH 
		
		
		pygame.draw.rect(screen, self.color, [self.x_point, self.y_point, self.width, self.height])
		#pygame.draw.rect(screen, BLUE, (width, height2), 0)
		#pygame.draw.rect(screen, BLUE, (width, height3), 0)
...
class Scroller(object):
	...
	def add_buildings(self):
		...
		"""
		left_width = 0
		while left_width <= self.width :
			self.add_building(left_width)
			left_width += self.buildings[-1].width
			self.scroll_end = left_width
	...
	def add_building(self, x_location):
		...
		"""
		width = random.randint(self.width//10, self.width//2)
		max_height = self.base - self.height
		height = random.randint (max_height//10, max_height//2)
		color = random.choice(colors)
		y_location = self.base - height
		self.buildings.append(Building(x_location, y_location, width, height, self.color))
	...
	def move_buildings(self):
		...
		location = self.buildings[-1].x_point + self.buildings[-1].width
		self.add_building(location)
